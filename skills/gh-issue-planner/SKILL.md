...
3. Ask **"do these ship together?"** — the answer picks the flow, not the dependency
   ...
   |---|---|---|
   | Yes, one release | **batch** | `gh-batch-runner` |
   | No, and they are independent | **individual** | `gh-issue-resolver` per Issue |
   | No, but one depends on another | **stack** | `gh-issue-resolver` with the dependency's branch as base |
